...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const values = [a, b].sort((j, k) => j - k);
  return `${isStartIncluded ? '[' : '('}${values[0]}, ${values[1]}${
    isEndIncluded ? ']' : ')'
  }`;
}
...
function getCommonDirectoryPath(pathes) {
  const pathsArr = pathes.map((el) => el.split('/'));
  const arrCommomDirectory = [];

  for (let i = 0; i < pathsArr[0].length; i += 1) {
    const cur = pathsArr[0][i];

    if (pathsArr.every((el) => el[i] === cur)) {
      arrCommomDirectory.push(pathsArr[0][i]);
    }
  }

  if (arrCommomDirectory.length > 0) {
